...
## Phase 7：示范轨迹提升训练精度（当前主线，App / Dev Client）

> PRD 0.3.2：优先 FR-067～069；详情解剖 3D 片（FR-064）P2 延后。

| 任务 ID | 任务 | 验收 |
|---------|------|------|
| T7-1 | 轨迹格式 + 离线提取（深蹲/俯卧撑各 ≥1） | FR-067；`trajectory-pipeline.md` |
| T7-2 | 训练页参考骨架按进度叠加（可开关） | FR-068；真机 VT-P7-002 |
| T7-3 | 轨迹辅助校验/阈值校准（矩阵同步） | FR-069；VT-RB + 真机抽测 |
| T7-4 | （延后）详情预渲染解剖 mp4 | FR-064 |

**本阶段验收**：深蹲或俯卧撑训练中，示范参考骨架可用且对照感优于纯手调关键帧。
